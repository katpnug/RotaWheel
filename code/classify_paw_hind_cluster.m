function [ BLOBclassified final_bbox stats] = classify_paw_hind_cluster(BLOB, BLOBclassified, pawID, PAWclass, dist_thresh, frame_iter, num_frames, noisy_bbox, stats, wheel_center, bboxColor)
%% constants
frame_name = BLOB.FRM;
...
p_hind_pos    = pP*mvnpdf(double(BLOBclassified.CENTROID{frame_iter}),[mu_xpos mu_ypos],cov_pos);
p_hind_area   = pA*normpdf(double(BLOBclassified.AREA{frame_iter}(:,1)),stats.mu_area,stats.sig_area);
BLOBclassified.logprob{frame_iter} = log([repmat(nan,[size(p_hind_area,1) 1]) p_hind_pos.*p_hind_area]);
paw_label = 2;
...
while ~isempty(notunique) && sum(~ismember(cc(notunique),4))>0 % while there are repeats and are not noise...
    ...
    while ~isempty(repeatedClass)
        ...
        % finding least prob for double classification option...to update
...
switch num_bbox
    ...
    otherwise
        % Goal: elimnate bbox to get down to 2!
        BLOBclassified.info(:,frame_iter) = ...
...
if size(BLOBclassified.FRONTclass,2) == size(BLOBclassified.HINDclass,2) && ...
    ...
    if front_xpos > hind_xpos
        confidence = 'ambiguous';
        % if PAWdist > dist_thresh, label FRONT as noise
        label = classLabel(4);
        color = bboxColor(4,:);
        final_bbox = insertObjectAnnotation(final_bbox,'rectangle',...
            BLOBclassified.FRONTclass{1,frame_iter},label,'color',color,'FontSize',FS,'linewidth',LW);
        BLOBclassified.FRONTclass(:,frame_iter) = [{BLOBclassified.BBOX{frame_iter}(find(cc==0),:)}; BLOB.FRM(frame_iter); {'turning'}; {confidence};...
            {BLOBclassified.logprob{frame_iter}(find(cc==0),:)}]; % front
...
if ismember([paw_label],cc(find(diag(BLOBclassified.logprob{frame_iter}(:,aa)) > LL_thresh==1)))
    % set confidence flag
    confidence = 'unambiguous';
    stats.dist_thresh = 75 + dist_const; %flip dist threshold b/c ordering (i.e. this will apply to hind
    ...
    % set confidence flag
    confidence = 'ambiguous';
    stats.dist_thresh = 100 + dist_const;
